...
if ($my_type == 'Download') {
    ...
    for ($i = 0; $i < count($avail_formats); $i++) {
        ...
        if ($config['VideoLinkMode'] == 'proxy' || $config['VideoLinkMode'] == 'both')
            echo ' / ' . '<a href="http://www.baneeishaque.somee.com/YouTube-Downloader/download.php?mime=' . $avail_formats[$i]['type'] . '&title=' . urlencode($my_title) . '&token=' . base64_encode($avail_formats[$i]['url']) . '" class="dl">download</a>';
        echo ') ' .
